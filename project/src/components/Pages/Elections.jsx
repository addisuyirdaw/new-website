...
// Countdown Timer Component - RESTORED
const CountdownTimer = ({ targetDate, onFinish }) => {
  const [timeLeft, setTimeLeft] = useState({
    hours: 0, minutes: 0, seconds: 0, finished: false
  });
  ...
  useEffect(() => {
    const calculate = () => {
      ...
      if (difference <= 0) {
        setTimeLeft({ hours: 0, minutes: 0, seconds: 0, finished: true });
  ...
  return (
    <div className="bg-amber-50 border-2 border-amber-200 rounded-2xl p-6 mb-8 flex flex-col items-center justify-center shadow-sm">
      <div className="flex items-center gap-2 text-amber-800 font-bold mb-3 uppercase tracking-widest text-xs">
        <Clock className="w-4 h-4" /> Voting Starts In:
      </div>
      <div className="flex gap-4">
        {[
          ...
          <div key={i} className="flex flex-col items-center">
            <div className="bg-white px-4 py-2 rounded-xl border-b-4 border-amber-200 text-2xl font-black text-amber-900 shadow-sm min-w-[60px] text-center">
              {String(unit.val).padStart(2, '0')}
            </div>
            <p className="text-[10px] uppercase font-black text-amber-500 mt-1">{unit.label}</p>
          </div>
        ))}
      </div>
    </div>
...
export function Elections() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Election Countdown & Global Status - RESTORED */}
        {(() => {
          const upcoming = elections.find(e => e.status === 'upcoming' || new Date(e.startDate) > new Date());
          if (upcoming) {
            return (
              <CountdownTimer
                targetDate={upcoming.startDate}
                onFinish={() => fetchElections()}
              />
            );
          }
          return null;
          ...
          <div className="mb-8 bg-white rounded-xl p-6 shadow-sm">
            ...
            {showNewElectionForm && (
              <div className="mt-6 space-y-6">
                <form id="electionForm" onSubmit={editingElectionId ? handleUpdateElection : handleCreateElection} className="space-y-4">
                  {/* Basic Election Info */}
                ...
                <motion.div
                  ...
                  <div className="flex space-x-3">
                    {election.status === "active" && (!user?.isAdmin || isSpecialAdmin) && (
                      (() => {
                        const isLocked = new Date() < new Date(election.startDate);
                        return (
                          <motion.button
                            whileHover={{ scale: (votedElections.has(election._id || election.id) || isLocked) ? 1 : 1.02 }}
                            whileTap={{ scale: (votedElections.has(election._id || election.id) || isLocked) ? 1 : 0.98 }}
                            onClick={() => setSelectedElection(election)}
                            className={`flex-1 py-2 px-4 rounded-lg font-medium transition-colors ${isLocked ? "bg-gray-200 text-gray-400 cursor-not-allowed border-2 border-gray-100" :
                              votedElections.has(election._id || election.id)
                                ? "bg-green-100 text-green-700 cursor-default"
                                : "bg-blue-600 text-white hover:bg-blue-700"
                              }`}
                            disabled={votedElections.has(election._id || election.id) || isLocked}>
                            {isLocked ? (
                              <>
